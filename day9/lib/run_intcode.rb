class RunIntcode
  attr_accessor :input
  attr_reader :halted, :phase, :output, :program, :pointer
  ...
  def param(position)
    @modes[-position]== 1 ?
      raw_param(position) :
      @program[position_index(position)]
  end

  def position_index(position)
    @modes[-position] == 2 ?
      raw_param(position) + @relative_offset :
      raw_param(position)
  end

  def raw_param(position)
    @program[@pointer + position] || 0
  end

  def set(position, value)
    @program[position_index(position)] = value
  end
end
